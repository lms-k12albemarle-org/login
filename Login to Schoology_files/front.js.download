// NOTE: This file is cached to see the changes after deploy it is needed to bypass the cache on inc_header_top.php.
...
$(document).ready(function(){
  ...
  loginForm.each(function(){
    ...
    var editSchoolInputObj = $('#edit-school'),
        ...
        schoolCookie = readCookie('s-js-login-selected-school');
    ...
    loginForm.bind('submit.cookie',function(){
      if( $('#remember-school').is(":checked") ) { // Did they check the option to remember the login?
        ...
        var schoolCookieObj = {
          ...
          'data'        : $("#edit-school-nid").data('selectedRow'),
        };

        // Create (or extend) login cookies for these values for 30 days
        if(schoolCookieObj.school_nid) {
          createCookie('s-js-login-selected-school',JSON.stringify(schoolCookieObj),30);
        }
      } else { // The option to remember their school isn't checked - so erase the cookies if they exist
        eraseCookie('s-js-login-selected-school');
      }
    });
    ...
    if(schoolCookie) {
      ...
      editSchoolNIDInputObj.data('selectedRow', schoolCookieObj.data); // Populate data attribute with the data from the cookie
    }
